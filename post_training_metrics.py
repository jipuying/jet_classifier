...
if __name__ == "__main__":
    ...
    prune_mask_set_small = {  # Float Model
        "fc1": torch.ones(38, 16),
        "fc2": torch.ones(11, 38),
        "fc3": torch.ones(22, 11),
        "fc4": torch.ones(5, 22)}
    prune_mask_set2 = [
        {  # 1/4 Quant Model
    ...
    try:
        if options.batnorm:
            loadmodel = models.three_layer_model_bv_tunable(prune_mask_set_small,[38,11,22], 6,
                                                            bn_affine=options.bn_affine, bn_stats=options.bn_stats)
        else:
            loadmodel = models.three_layer_model_bv_tunable(prune_mask_set_small, [38,11,22], 6)
    ...
    for model_bops, model_file in sorted(quant_model_set_6b.items()):
        if options.batnorm:
            ...
            models.three_layer_model_bv_tunable(prune_mask_set_small, [38,11,22], 6)
    ...
    aiq_ax.scatter([key for key in quant_6b_AiQ], [z['AiQ'] for z in quant_6b_AiQ.values()], c = 'blue', s = 6, label='6b Quantized')
    ...
    aiq_ax.scatter([key for key in quant_6b_AiQ], [z['auc_roc'] for z in quant_6b_AiQ.values()], c = 'blue', s = 6, label='6b Quantized')
    ...
    aiq_ax.scatter([key for key in quant_6b_AiQ], [z['accuracy'] for z in quant_6b_AiQ.values()], c = 'blue',s = 6,
                   label='6b Quantized')
    ...
    eff_ax.scatter([key for key in quant_6b_AiQ], [z['net_efficiency'] for z in quant_6b_AiQ.values()],c = 'blue',s = 6,
                label='6b Quantized')
    ...
    for label in test_dataset.labels_list:
        ...
        aiq_ax.scatter([key for key in quant_6b_AiQ], [z['sel_bkg_reject'][label] for z in quant_6b_AiQ.values()],c='blue',s = 6,
                    label='6b Quantized')
